...
import { allProperties, propertiesData } from "@/data/properties";
...
export default function Home() {
  ...
  return (
    <>
      ...
      {(() => {
        const videoProperties = Object.entries(propertiesData)
          .filter(([, p]) => p.videoEmbed && p.socialLinks)
          .map(([slug, p]) => ({ slug, ...p }));
            ...
            <div className="max-w-7xl mx-auto px-6 lg:px-10">
              <SectionReveal>
                <div className="text-center mb-16">
                  ...
                  <p className="mt-4 text-base text-body leading-relaxed max-w-2xl mx-auto">
                    Take a virtual walkthrough of our featured listings. Watch on your preferred platform.
                  </p>
                </div>
              </SectionReveal>

              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {videoProperties.map((vp, i) => (
                  <SectionReveal key={vp.slug} delay={i * 0.1}>
                    <div className="group bg-background-secondary rounded-lg overflow-hidden shadow-sm hover:shadow-lg transition-shadow duration-300">
                      <div className="aspect-[9/16] max-h-[480px] w-full bg-foreground/5">
                        <iframe
                          src={`${vp.videoEmbed}?rel=0&modestbranding=1`}
                          className="w-full h-full"
                          allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                          allowFullScreen
                          title={`${vp.title} - Video Tour`}
                        />
                      </div>
                      <div className="p-6">
                        <p className="text-xs font-body tracking-widest uppercase text-muted mb-1">{vp.location}</p>
                        <Link href={`/properties/${vp.slug}`} className="block">
                          <h3 className="font-heading text-lg text-foreground group-hover:text-body transition-colors duration-300">
                            {vp.title}
                          </h3>
                        </Link>
                        <p className="mt-1 text-sm text-body">{vp.configuration} &middot; {vp.sqft} Sq Ft</p>
                        {vp.socialLinks && (
                          <div className="mt-4 flex items-center gap-3">
                            {vp.socialLinks.youtube && (
                              <a href={vp.socialLinks.youtube} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full border border-border flex items-center justify-center text-muted hover:bg-[#FF0000] hover:text-white hover:border-[#FF0000] transition-all duration-300">
                                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
                              </a>
                            )}
                            {vp.socialLinks.facebook && (
                              <a href={vp.socialLinks.facebook} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full border border-border flex items-center justify-center text-muted hover:bg-[#1877F2] hover:text-white hover:border-[#1877F2] transition-all duration-300">
                                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                              </a>
                            )}
                            {vp.socialLinks.instagram && (
                              <a href={vp.socialLinks.instagram} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full border border-border flex items-center justify-center text-muted hover:bg-gradient-to-r hover:from-[#833AB4] hover:via-[#FD1D1D] hover:to-[#F77737] hover:text-white hover:border-transparent transition-all duration-300">
                            ...
                            <span className="text-xs text-muted ml-1">Watch on</span>
                          </div>
                        )}
                      </div>
                    </div>
